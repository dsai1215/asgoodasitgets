...
\textbf{Case 1: Uniform perturbations}
...
\begin{align}
& \dfrac{9}{4 \tau}  \left((d_n^l)^2-\lambda\right) \left(\sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n)\right. \nonumber\\
&\hspace{6em}\left. + 2\sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N d_n^i d_n^j \nabla^2_{ij} f(x_n)\right)\nonumber\\
=& \dfrac{9}{4 \tau} (d_n^l)^2 \sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \nonumber\\
& + \dfrac{9}{2\tau} (d_n^l)^2\sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N d_n^i d_n^j \nabla^2_{ij} f(x_n)\nonumber\\
& - \dfrac{9 \lambda}{4\tau} \sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) - \dfrac{18 \lambda}{2 \tau} \sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N d_n^i d_n^j \nabla^2_{ij} f(x_n).\label{eq:h2}
\end{align}
...
\begin{align*}
&\dfrac{9 \lambda}{4\tau} \E\left(\left. \sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right| \F_n\right)\\
= &\dfrac{9 \lambda}{4\tau} \sum\limits_{i=1}^N \E \left[(d_n^i)^2\right] \nabla^2_{ii} f(x_n) 
=  \dfrac{5}{4} \sum\limits_{i=1}^N \nabla^2_{ii} f(x_n), \text{ a.s.}
\end{align*} 
Combining the above followed by some algebra, we obtain 
\begin{align*}
&\dfrac{9}{4\tau}  \E\left[\left. \left((d_n^l)^2-\lambda\right) \left(\sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right.\right.\right.\\
...
\begin{align}
& \dfrac{9}{10\tau} \E\left[\left.d_n^k d_n^l   \left(\sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right.\right.\right. \nonumber\\
&\hspace{3em}\left.\left.\left.+ 2\sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N d_n^i d_n^j \nabla^2_{ij} f(x_n)\right)\right| \F_n \right]\nonumber\\
=& \dfrac{9}{10 \tau} \sum\limits_{i=1}^N \E \left(d_n^k d_n^l (d_n^i)^2 \right)\nabla^2_{ii} f(x_n) \nonumber\\
& \quad+ \dfrac{9}{5 \tau}\sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N \E\left(d_n^k d_n^l d_n^i d_n^j\right) \nabla^2_{ij} f(x_n) \label{eq:crossh}\\
= & \nabla^2_{kl} f(x_n).\nonumber
\end{align}
The last equality follows from the fact that the first term in \eqref{eq:crossh} is $0$ since $k\ne l$, while the second term in \eqref{eq:crossh} can be seen to be equal to $\dfrac{9}{10 \tau} \E\left((d_n^k)^2 (d_n^l)^2\right) \nabla^2_{kl} f(x_n) = \nabla^2_{kl} f(x_n)$.
The claim follows for the case of uniform perturbations.\ \\\ \\

\textbf{Case 2: Asymmetric Bernoulli perturbations}

Note that the proof up to \eqref{eq:h1} is independent of the choice of perturbations. The proof differs in the analysis of the diagonal and off-diagonal terms in \eqref{eq:h1}. In the case of asymmetric Bernoulli perturbations, the normalizing scalars in the definition of $M_n$ in \eqref{eq:2rdsa-estimate-ber} are different. 

\subsection*{Diagonal terms in \eqref{eq:h1}}
Let $\tau = \dfrac{(1+\epsilon)(1+(1+\epsilon)^3)}{(2+\epsilon)}$ denote the fourth moment $E (d_n^i)^4$, for any $i=1,\ldots,N$.
An analogue of \eqref{eq:h2} is as follows:
\begin{align}
& \frac{1}{\tau (1- \frac{\lambda^2}{\tau })}  \E\left(\left. \left((d_n^l)^2-\lambda\right) \left(\sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right.\right.\right. \nonumber\\ 
&\hspace{8em}\left.\left.\left.+ 2\sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N d_n^i d_n^j \nabla^2_{ij} f(x_n)\right)\right| \F_n\right)\nonumber\\
=& \frac{1}{\tau (1- \frac{\lambda^2}{\tau })} \E\left(\left. (d_n^l)^2 \sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right| \F_n\right) \nonumber\\
& - \frac{\lambda}{\tau (1- \frac{\lambda^2}{\tau })} \E\left(\left. \sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right| \F_n\right) \label{eq:h2-a}
\end{align}
We have used the fact that the second term in the LHS above is conditionally zero  (see argument below \eqref{eq:h2} for a justification).
The first term on the RHS of \eqref{eq:h2-a} be simplified as follows:
\begin{align}
&\frac{1}{\tau (1- \frac{\lambda^2}{\tau })} \E\left(\left. (d_n^l)^2 \sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right| \F_n\right)\nonumber\\
= & \frac{1}{\tau (1- \frac{\lambda^2}{\tau })} \E\bigg((d_n^l)^4 \nabla^2_{ll} f(x_n)  \nonumber\\
&\qquad + \sum\limits_{i=1,i\ne l}^N (d_n^l)^2(d_n^i)^2 \nabla^2_{ii} f(x_n) \bigg)\nonumber\\
= & \frac{\lambda^2}{\tau (1- \frac{\lambda^2}{\tau })} \left( \nabla^2_{ll} f(x_n) + \dfrac{(1+\epsilon)^2}{\phi}\sum\limits_{i=1,i\ne l}^N  \nabla^2_{ii} f(x_n) \right).\label{eq:diag1}
\end{align} 
For the second equality above, we have used the fact that $\E[(d_n^l)^4] = \tau$ and $\E[(d_n^l)^2 (d_n^i)^2] = \E[(d_n^l)^2] \E[(d_n^i)^2] = \lambda^2$, $\forall l \ne i$.

The second term in \eqref{eq:h2-a} with the conditional expectation and without the negative sign can be simplified as follows:
\begin{align}
&\frac{\lambda}{\tau (1- \frac{\lambda^2}{\tau })} \E\left(\left. \sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right| \F_n\right)\nonumber\\
= & \frac{\lambda}{\tau (1- \frac{\lambda^2}{\tau })} \sum\limits_{i=1}^N \E \left[(d_n^i)^2\right] \nabla^2_{ii} f(x_n) \nonumber\\
= &\frac{\lambda^2}{\tau (1- \frac{\lambda^2}{\tau })} \sum\limits_{i=1}^N \nabla^2_{ii} f(x_n).\label{eq:diag2}
\end{align} 
Combining \eqref{eq:diag1} and \eqref{eq:diag2}, the correctness of the Hessian estimate follows for the diagonal terms.

\subsection*{Off-diagonal terms in \eqref{eq:h1}}
Consider the $(k,l)$th term in \eqref{eq:h1}, with $k<l$. We obtain
\begin{align}
& \dfrac{1}{2 \lambda^2} \E\left[\left.d_n^k d_n^l   \left(\sum\limits_{i=1}^N (d_n^i)^2 \nabla^2_{ii} f(x_n) \right.\right.\right. \nonumber\\
&\qquad\qquad \left.\left.\left. + 2\sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N d_n^i d_n^j \nabla^2_{ij} f(x_n)\right)\right| \F_n \right]\nonumber\\
=& \dfrac{1}{2 \lambda^2} \sum\limits_{i=1}^N \E \left(d_n^k d_n^l (d_n^i)^2 \right)\nabla^2_{ii} f(x_n) \nonumber\\
&\quad+ \dfrac{1}{2 \lambda^2}\sum\limits_{i=1}^{N-1}\sum\limits_{j=i+1}^N \E\left(d_n^k d_n^l d_n^i d_n^j\right) \nabla^2_{ij} f(x_n) \label{eq:crossh-a}\\
= & \nabla^2_{kl} f(x_n).\nonumber
\end{align}
Note that the first term on the RHS of \eqref{eq:crossh-a} equals zero since $k\ne l$.
The claim follows for the case of asymmetric Bernoulli perturbations.
