...
\begin{tikzpicture}[thick]
  \node  at ( 0,0) [circle,draw=green!90,fill=green!80,label=below:$\dfrac{(1+\epsilon)}{(2+\epsilon)}$] {-1};
  \node  at ( 2,0) [circle,draw=green!90,fill=green!80,label=below:$\dfrac{1}{(2+\epsilon)}$] {$1+\epsilon$};
  \draw (0.35,0) --  (1.4,0);
 \end{tikzpicture}
 \end{center}
...
% \bottomrule
 \end{tabular}}
\end{table}
\footnotetext[1]{2RDSA-Unif uses $Unif [-1,1]$ with different $M_n$.}
 \end{small}
\end{frame}
